...
function AtImport(optionsUser) {
    ...
    // debugger;
        ...
        return parseStyle(result, styles, options).then((mergeModules) => {
            const selectors = {};
            if (mergeModules.length === 0)
                return;
            ...
            for (let i = mergeModules.length - 1; i >= 0; i--) {
                const module = mergeModules[i].root;
...
function parseStyle(result, styles, options) {
    ...
    nodes.forEach((node) => {
        let mergeModule;
        if (node.type === 'atrule' && node.name === 'merge') {
...
module.exports = postcss.plugin('postcss-merge', AtImport);
