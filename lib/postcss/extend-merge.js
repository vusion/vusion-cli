...
function merge(root) {
    ...
    const extendModules = getExtendModules(root);
    if (!extendModules)
        return root;
    addRawModules(root);
    const extendSelectors = getExtendSelectors(extendModules);
    root.extendSelectors = extendSelectors;
    extendRules(root.raw, extendSelectors);
    return root;
}
...
function extendRules(styles, extendModules) {
    const hasExtendSelectors = {};
    styles.walkRules((rule) => {
        const selectorNames = getSelector(rule);
        selectorNames.forEach((selectorName) => {
            if (extendModules.hasOwnProperty(selectorName) && !hasExtendSelectors.hasOwnProperty(selectorName)) {
                hasExtendSelectors[selectorName] = true;
                rule.removeSelf = true;
                const hasSelector = extendModules[selectorName];
...
function removeMark(styles) {
    const markList = [EXTEND_START, EXTEND_END, IMPORT_START, IMPORT_END];
...
function isPair(start, end) {
    if (start.text === EXTEND_START && end.text === EXTEND_END)
        return true;
...
module.exports = postcss.plugin('postcss-vusion-extend-merge', postcssPlugins);
