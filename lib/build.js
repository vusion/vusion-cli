...
const prepare = (webpackConfig) => {
    ...
    const svgRule = {
        test: /\.svg$/,
        oneOf: [{ loader: 'file-loader', options: { name: '[name].[hash:16].[ext]' } }],
    };

    if (config.extractCSS) {
        plugins.push(new SpriteLoaderPlugin());
        svgRule.oneOf.unshift({ resourceQuery: /sprite/, loader: 'svg-sprite-loader', options: { extract: true, spriteFilename: 'sprite.[hash:16].svg' } });
    }
