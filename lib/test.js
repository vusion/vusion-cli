...
const prepareRequireTest = () => {
    ...
    return { srcPath, testPath, requireTestPath };
};

const prepare = (webpackConfig, port, files) => {
    /**
     * Webpack config
     */
    ...
    let requireTestPath = path.resolve(process.cwd(), files);
    if (!requireTestPath)
        requireTestPath = prepareRequireTest().requireTestPath;
...
module.exports = (webpackConfig, files) => ({
    start() {
        checkPort(config.karma.port || 9876, 'localhost')
            .then((port) => {
                const karmaConfig = prepare(webpackConfig, port, files);
