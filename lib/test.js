...
const prepareRequireTest = () => {
    ...
    const testPath = path.join(process.cwd(), getTestPath());
    const vueFilesWithTest = filterVueTest(testPath);
    ...
    if (vueFilesWithTest.length) {
        const regPaths = vueFilesWithTest.map((filePath) => path.relative(testPath, filePath).replace(/([\\/.])/g, '\\$1')).join('|');
        const replacer = `const vueContext = require.context('!!${selectorPath}?type=customBlocks&index=0!${testPath}', true, /${regPaths}/);
                          vueContext.keys().forEach(vueContext);`;
        requireTest = requireTest.replace('/* VUECONTEXT_REPLACER */', replacer);
    }
    requireTest = requireTest.replace('testSrcPath', `'${testPath}'`).replace('testPath', `'${path.join(process.cwd(), './test/unit')}'`);
